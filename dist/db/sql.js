...
exports.saveToSql = void 0;
// 导入数据库操作模块
var index_1 = require("./index");
function saveToSql(Items) {
    console.log("开始insert SQL操作");
    var sqlStr = "insert into markdata(postid,blogger,articeltime,commentscount,repostscount,likecount,piccount,articel,totalmark,bloggeruid,interactive) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
    if (Items.length == 0) {
        console.log('评论数为0');
        return;
    }
    for (var i = 0; i < Items.length; i++) {
        ...
        index_1.db.query(sqlStr, modSqlParams, function (err, results) {
            if (err)
